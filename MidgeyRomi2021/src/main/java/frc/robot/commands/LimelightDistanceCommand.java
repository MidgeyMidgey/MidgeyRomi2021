...
public class LimelightDistanceCommand extends CommandBase {
  ...
  private static final double TARGET_AREA = 200; // 1.481;
  private static final double MAX_AREA_ABS_ERROR = 10; //0.005;

  private RomiDrivetrain m_robotDrive;
  private LimelightSubsystem m_limelight;

  public LimelightDistanceCommand(LimelightSubsystem limelight, RomiDrivetrain robotDrive) {
  ...
  public void execute_old() {
    double minValue = 0.1; //0.001
    double speed = 0.0;
    double heading = 0.0;
    if (m_limelight.hasTarget()){
        speed = (TARGET_AREA - m_limelight.getArea()) * 0.10;
        if(speed > 0){
          speed += minValue;
        } else if (speed < 0){
          speed -= minValue;
